...
const NewUserRecipe = () => {
    ...
    const { register: registerRecipe, handleSubmit: handleSubmitRecipe, formState: formStateRecipe, control, formState: { errors } } = useForm<RecipeMainData>();
    ...
    const units = [
      ...
      const onSubmit: SubmitHandler<RecipeMainData> = async (data: RecipeMainData) => {
        try {
            ...
            const result = await APIpostAuth("Recipes",requestData);    
            setSuccessfullMessage("Successfully created!");
            await sleep(1000);
